...
module.exports = function(app, usermanip, sendMail) {
  ...
  function _sendEmail(action, user, req, cb) {
    var token = createToken(action, user);

    var serverAddr = process.env.SERVERURL ||
      ...
      usermanip.find(decoded.email, function(err, user) {
        ...
        usermanip.save(user, function(err, saved) {
          ...
          if(process.env.SET_PWD_AFTER_VERIFICATION) {
            var token = createToken('reset', user);
            var url = (process.env.CLIENTAPPURL || '') + '/changepwd?sptoken=' + token;
