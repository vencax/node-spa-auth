...
const CHPWDURL = process.env.CHANGE_PASSWORD_URL
...
module.exports = function (usermanip, sendMail, createError) {
  ...
  function _serverAddress (req) {
    return process.env.SERVERURL || req.protocol + '://' + req.get('host')
  }

  function _sendEmail (action, user, req) {
    const token = createToken(action, user)
    const serverAddr = _serverAddress(req)
