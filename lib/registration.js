...
module.exports = function(app, usermanip, sendMail) {
  ...
  function _sendEmail(action, user, req, cb) {
    var content = {action: action, email: user.email};fallback template loading
    var token = jwt.sign(content, process.env.SERVER_SECRET, {
      expiresInMinutes: process.env.EMAIL_VALIDATION_TOKEN_DURATION || 24 * 60
    });
    var chpasslink = process.env.CHPASSWDLINK | '/changepasswd';
    var linkAct = (action === 'reset') ? chpasslink : '/auth/userverify';
    var ctx = {
      fullName: user.fullName,
      project: process.env.PROJECT_NAME || req.hostname,
      link: req.headers.origin + linkAct + '?sptoken=' + token
    };
    try {
      ctx.lang = req.headers["accept-language"].split(';')[0].split(',')[0];
    } catch(e) {}
    emailing.send(action, user.email, ctx, sendMail, cb);
  }
  ...
  app.get('/uservefify', function(req, res) {
