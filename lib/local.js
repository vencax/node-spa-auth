...
module.exports = (usermanip, createError) => (req, res, next) => {
  ...
  .then(user => {
    ...
    let pwdValid = Utils.validPassword(user, req.body.password)
    pwdValid = !pwdValid && usermanip.alterPasswordValidator
      ? usermanip.alterPasswordValidator(req.body.password)
      : pwdValid
    if (!pwdValid) {
      return next(createError('incorrect credentials', 401))
    }
